		...
		if(!empty($custom_img_dirPath)):
			...
			{						
				...
				if(extension_loaded('gd')){
					...
					list($CurWidth, $CurHeight) = getimagesize($imgUrl);
	...
	if(isset($params) && !empty($params) && is_array($params)):
		...
		foreach($params as $key => $value):
			...
			if (!empty($custom_imgurl) && filter_var($custom_imgurl, FILTER_VALIDATE_URL) ):
					...
					{						
						...
						if(extension_loaded('gd')){
							...
							if(false===$sizes){
								update_option($key,$custom_imgurl);
								$response['res'] = 'success';
								$response['warning'] = 'Couldn\'t Crop the icons';
							}else{
							...
							update_option($key,$custom_imgurl);
							$response['res'] = 'success';
							$response['warning'] = 'Gd libraby not loaded';
						}
					}
				...
				$response['res'] = 'Invalid image url';
...
function plus_UploadMouseOverIcons(){
	...
	$icons  = ($option1['sfsi_custom_desktop_icons']) ? unserialize($option1['sfsi_custom_desktop_icons']) : array();
		...
		function($iconName,$iconUploadPathToSave,$customIconIndex){
			...
			if(isset($iconUploadPathToSave) && !empty($iconUploadPathToSave)){
				...
				$option3 =  unserialize(get_option('sfsi_premium_section3_options',false));
			  	...
			  	$arrMouseoverIcons =  (isset($option3['sfsi_plus_mouseOver_other_icon_images']))? unserialize($option3['sfsi_plus_mouseOver_other_icon_images']): array();
			...
			$response['message'] = 'Icon sucessfully uploaded';
			$response['url']     = SFSI_PLUS_UPLOAD_DIR_BASEURL.$iconUploadPathToSave;
...
function sfsi_premium_deleteIcons(){
	...
	$response = array('status'=> false, "message" => "Failed to delete the icon");
	...
	if(isset($icon_name) && !empty($icon_name) && isset($serviceType) && !empty($serviceType)){
		...
		switch ($serviceType) {
			...
			case 'mouseover':
				...
				$response['status']      = true;
				$response['message']     = "Icon successfully deleted";
...
function sfsi_plus_get_Imdone_returnHtml($iconName,$classNumber,$bgPosition){
	...
	if(isset($iconName) && !empty($iconName)){
		...
		if($iconUrl)
		{
			...
			$rhtml = '<span class="sfsiplus_row_17_'.$classNumber.' sfsiplus_'.$iconName.'_section. sfsi_plus-bgimage" style="background: url('.$icon.') no-repeat; margin-top: 6px;">		
			</span>';
		}
		else
		{
...
function sfsi_plus_Iamdone()
{
	...
	$iconsArr =  array(
		...
		array("iconName" => "google", 	 "classNumber"  => 4,  "bgPosition" => "-145px 6px"),
...
function sfsi_plus_remove_custom_icon_mouseover_icon($customIconIndex){
   ...
   $option3 = (get_option('sfsi_premium_section3_options',false)) ? unserialize(get_option('sfsi_premium_section3_options',false)) : array() ;
...
function sfsi_plus_resize_gif($srcImageDirPath,$width,$height,$outImageDirPath){
	...
	$imageCroppedResult->message = "Something went wrong";
		...
		&& isset($height) && !empty($height) && isset($outImageDirPath) && !empty($outImageDirPath) )
	{
		if(extension_loaded('imagick') ){
			...
			//catch exception
		 	...
		 	$imageCroppedResult->message = "Image Magick extension is required for cropping gif images";
