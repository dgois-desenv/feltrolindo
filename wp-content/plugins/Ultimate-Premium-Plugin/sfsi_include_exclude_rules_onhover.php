    ...
    if($anySettingSelected){
        ...
        if($is_archive){
            ...
            if ($is_tag){
                ...
                $arrSfsi_plus_list_exclude_taxonomies = (isset($option5['sfsi_plus_icon_hover_list_exclude_taxonomies'])) ? unserialize($option5['sfsi_plus_icon_hover_list_exclude_taxonomies']) : array();
...
function sfsi_plus_onhover_icon_include($url,$is_archive=null,$is_date=null,$is_author=null)
{
    ...
    if($anySettingSelected){
        // var_dump([$postid,is_single([$postid]),is_single(),$post]);
        if($is_archive){
            ...
            if ($is_tag){
                ...
                //$arrSfsi_plus_list_include_taxonomies = (isset($option5['sfsi_plus_icon_hover_list_include_taxonomies'])) ? unserialize($option5['sfsi_plus_icon_hover_list_include_taxonomies']) : array();

                $arrIncludeTx = array_filter($option5['sfsi_plus_icon_hover_list_include_taxonomies']);

                if(!empty($arrIncludeTx)){
                    
			//var_dump($arrIncludeTx);
                   // $termData = get_queried_object();
			...
			foreach($custom_taxonomies as $custom_tax){
				if(in_array($custom_tax->taxonomy,$arrIncludeTx)){
					//var_dump($custom_tax,$custom_tax->term_taxonomy_id,get_term_link($custom_tax->term_taxonomy_id),$url);
...
//	else if ($option5['sfsi_plus_icon_hover_switch_include_taxonomies'] == 'yes'){
//		$custom_taxonomy=get_terms(array(
		//	'taxonomy' => $option5['sfsi_plus_icon_list_include_taxonomies'],
//			'hide_empty' => false
//		));
		//var_dump($custom_taxonomy);
//	}
        else
        {
            if($switchInclude == 'yes')
